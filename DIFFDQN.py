...
class DQN_Network(nn.Module):
    def __init__(self, state_shape=STATE_SHAPE, action_shape=ACTION_SHAPE):
        super(DQN_Network, self).__init__()
        self.fc1 = nn.Linear(state_shape, 512)
        self.fc2 = nn.Linear(512, 256)  
        self.fc3 = nn.Linear(256, 64)
        self.fc4 = nn.Linear(64, action_shape)
...
def Experiment(max_episodes=MAX_EPISODES, max_steps=MAX_STEPS):
    ...
    for lr in learning_rates:
        agent = Agent(lr=lr)  
        strategy_rewards[lr] = []
        for episode in tqdm(range(max_episodes), desc=f"Training with lr={lr}"):
            state = ENV.reset()
            total_reward = 0
            for step in range(max_steps):
                action = agent.select_action(state, policy='egreedy', epsilon=epsilon, temp=1.0)
            ...
            epsilon = max(epsilon * DECAY_EXPLORATION, MIN_EXPLORATION)
            agent.update_target()
            strategy_rewards[lr].append(total_reward)

    plt.figure()
    for lr, rewards in strategy_rewards.items():
        average_rewards = np.cumsum(rewards) / (np.arange(max_episodes) + 1)
        plt.plot(average_rewards, label=f'LR={lr}')

    plt.xlabel('Episode')
    plt.ylabel('Average Reward')
    plt.title('Average Reward by Learning Rate')
    plt.legend()
    plt.savefig('CompareDQN.png')
    plt.show()
