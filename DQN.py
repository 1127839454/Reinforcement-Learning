...
class Agent():
    ...
    def save_model(self, strategy):
        path = os.path.join('model', '_'.join(['DQN', strategy, "model.pth"]))   
...
def Experiment(env, agent, strategies, max_episodes=MAX_EPISODES, max_steps=MAX_STEPS):
    ...
    plt.title('Average Reward by different Strategies about DQN')
    plt.legend()
    plt.savefig('DQN.png')
    plt.show()
...
def test_agent(env, agent, strategies, episodes=10):
    for strategy in strategies:
        model_path = os.path.join('model', '_'.join(['DQN', strategy, "model.pth"]))   
